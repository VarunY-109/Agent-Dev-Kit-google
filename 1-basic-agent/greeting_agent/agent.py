...
# `SubAgent`, or `Session`), add them to this section so reviewers can
# quickly see the module's external dependencies.

# =====================================================================
# Constants
# =====================================================================

#: str: The machine-friendly identifier for this agent. It is used by the
#: ADK runtime for logging, agent dispatch, and as part of any generated
#: trace or session record. Keep it stable across releases to avoid
#: breaking saved sessions.
AGENT_NAME = "greeting_agent"

#: str: The identifier of the underlying generative model. Gemini 2.0 Flash
#: was selected for its low-latency responses, which makes it well-suited
#: for a conversational greeting scenario where snappy replies matter
#: more than deep reasoning. See the Gemini model catalog for a list of
#: alternatives if you need different latency/quality trade-offs.
AGENT_MODEL = "gemini-2.0-flash"

#: str: A short, human-readable description of the agent's purpose. The
#: ADK runtime surfaces this string in agent listings, developer UIs, and
#: auto-generated documentation. Keep it concise (a single sentence).
AGENT_DESCRIPTION = "Greeting agent"

#: str: The natural-language system prompt that guides the model's
#: behavior. Multi-line strings are preserved verbatim, including the
#: leading whitespace and newlines, so the model receives the formatted
#: instructions exactly as authored below.
#:
#: The instruction is deliberately short to minimize token usage on
#: every turn. If you find the model straying from the desired behavior,
#: consider expanding the instruction with explicit examples rather than
...
#
# Arguments:
#     name (str): The unique identifier for the agent within its module.
#         Must be a valid Python identifier-style string and should remain
#         stable across versions to preserve session compatibility.
#     model (str): The fully-qualified model identifier. Must be a model
#         available to your project's credentials; otherwise the agent
#         will fail at first invocation with an authentication error.
#     description (str): A one-sentence summary of the agent's purpose.
#         Used for documentation listings and developer tooling.
#     instruction (str): The system prompt. May include newlines and
#         leading/trailing whitespace; the ADK passes it through to the
#         model without modification.
#
# Returns:
#     Agent: A configured agent instance. The object is ready to be
#     served by `adk run` or `adk api_server` and requires no further
#     setup.
#
# Raises:
#     ImportError: If the `google.adk.agents` package is not installed.
#         Install it via `pip install google-adk` and retry.
#     pydantic.ValidationError: If any of the constructor arguments fail
#         ADK's schema validation (for example, an empty `name`).
#
...
# TODO: Add a unit test under `tests/test_greeting_agent.py` that uses
# the ADK's in-memory runner to verify the greeting flow end-to-end.
# TODO: Explore wiring up a custom tool (e.g., `set_user_name`) to
# persist the user's name in a session-scoped state object.
...
# runtime) will treat `root_agent` as the only intended export.
__all__ = ["root_agent"]
